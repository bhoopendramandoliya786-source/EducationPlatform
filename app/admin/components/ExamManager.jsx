...
import { Award, Plus, Trash2, CheckCircle, BookOpen, RefreshCw } from 'lucide-react';
...
export default function ExamManager() {
  ...
  const [examSubjects, setExamSubjects] = useState({}); // { exam_id: [subject_id, ...] }
  ...
  async function handleAddExam(e) {
    ...
    try {
      ...
      const { data, error } = await supabase.from('exams').insert([
        { name, slug: slug.toLowerCase().trim(), category, description, sort_order: exams.length + 1 }
      ]).select().single();

      if (error) throw error;

      // Auto-link all active subjects by default
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 2. Existing Exams & Subject Mapping */}
            ...
            <div key={exam.id} className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/90 space-y-3 shadow-lg">
              ...
              <div className="pt-3 border-t border-slate-800/80 space-y-1.5">
                <span className="text-[11px] font-bold text-slate-300 flex items-center gap-1.5">
                  ...
                  इस परीक्षा में शामिल विषय चुनें (Select Subjects):
                </span>
