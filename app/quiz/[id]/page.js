...
export default function QuizPlayPage() {
  ...
  if (loading) {
    return (
      <div className="h-screen bg-[#111216] text-white flex flex-col items-center justify-center gap-3">
  ...
  if (!questions.length) {
    return (
      <div className="h-screen bg-[#111216] text-white flex flex-col items-center justify-center p-6 text-center space-y-4">
        <p className="text-zinc-400 text-sm">इस टॉपिक के लिए अभी कोई प्रश्न उपलब्ध नहीं हैं।</p>
        <button
          onClick={() => router.back()}
          className="px-6 py-2.5 bg-blue-600 rounded-xl text-xs font-bold"
        >
          वापस जाएँ
        </button>
      </div>
  ...
  if (quizFinished) {
    ...
    return (
      <div className="h-screen bg-[#111216] text-zinc-100 flex flex-col items-center justify-center p-5 font-sans">
        <div className="w-full max-w-md bg-[#1a1c22] border border-zinc-800 rounded-3xl p-6 sm:p-8 text-center space-y-6 shadow-2xl">
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 py-3.5 bg-zinc-800 hover:bg-zinc-700 rounded-xl text-xs font-bold text-zinc-200 transition flex items-center justify-center gap-2 cursor-pointer"
            >
  ...
  return (
    <div className="min-h-[100dvh] bg-[#111216] text-zinc-100 font-sans flex flex-col justify-between selection:bg-blue-500/30">
      ...
      <header className="w-full max-w-xl mx-auto px-4 pt-3 pb-2 shrink-0">
        ...
        {/* Dashed Progress Line + Score Pills */}
        <div className="mt-2.5 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0 text-xs font-bold">
            ...
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-rose-500/20 text-rose-300 border border-rose-500/30 text-[11px]">
      ...
      {/* 2. CENTER CONTENT: FULL CARD CANVAS */}
      <main className="w-full max-w-xl mx-auto px-4 py-4 flex-1 flex flex-col justify-start space-y-4">
        ...
        <div className="space-y-3 pt-1">
          {['A', 'B', 'C', 'D'].map((optKey) => {
            ...
            if (isAnswered) {
              if (isCorrect) {
                cardStyle = "bg-[#11271c] border-emerald-500 text-emerald-200 shadow-lg shadow-emerald-950/40";
              } else if (isChosen) {
                cardStyle = "bg-[#2d151c] border-rose-500 text-rose-200 shadow-lg shadow-rose-950/40";
              } else {
                cardStyle = "bg-[#16171c]/70 border-zinc-800/40 text-zinc-500 opacity-60";
              }
            }
            ...
            return (
              ...
              >
                ...
                {isAnswered && (isCorrect || isChosen) && (
                  <div className="pt-2.5 border-t border-white/10 text-xs leading-relaxed">
      ...
      <footer className="w-full max-w-xl mx-auto py-4 px-4 shrink-0">
        <div className="flex items-center justify-center gap-3">
          <button
            onClick={handleBack}
            disabled={currentIndex === 0}
            className="px-8 py-3.5 rounded-full bg-[#1c1e24] hover:bg-zinc-800 disabled:opacity-30 disabled:pointer-events-none text-zinc-300 text-xs font-bold transition cursor-pointer border border-zinc-800"
          >
            Back
          </button>
          <button
